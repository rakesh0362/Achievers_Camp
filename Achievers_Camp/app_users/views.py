...
def user_login(request):
    if request.method == "POST":
        ...
        if user:
            if user.is_active:
                login(request, user)
                return HttpResponseRedirect(reverse('index')+'#list-course')
            else:
...
def emailContactForm(request):
    ...
    content = "Name: " + str(name) + " email id: " + str(email) + \
        " Mobile No. " + str(contactid) + "Exam perparing for " + str(examid)
    if request.method == 'POST':
        send_mail(subject, content, settings.EMAIL_HOST_USER, [
                  'info@achieverscamp.com'], fail_silently=False)
